...
if __name__ == '__main__':
	...
	spark = SparkSession.builder \
			.appName('RLab_Stats_Report___Extract_Active_Devices') \
	...
	active_androids.where(active_androids.flag.isNull()).drop('flag').sample(False, 0.05, 1003).repartition(50).\
		write.csv('./hgy/rlab_stats_report/sampled_devices/{0}'.format(args.query_month), header=True)
	...
	##print(active_ios.count())
	##print(active_ios.select('idfa').distinct().count())
	...
	result.repartition(1).write.csv('./hgy/rlab_stats_report/active_devices/{0}'.format(args.query_month), header=True)